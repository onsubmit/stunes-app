...
function AuthorizeForm() {
  const [showConnectButton, setShowConnectButton] = useState<boolean | undefined>();
  const [profileBadgeProps, setProfileBadgeProps] = useState<ProfileBadgeProps | undefined>();
  ...
  useEffect(() => {
    ...
    getProfileBadgePropsAsync().then((result) => {
      if (result.ok) {
        setProfileBadgeProps(result.val);
      } else {
    ...
    getOrRefreshAccessTokenAsync().then((result) => {
      if (result.ok) {
        getMeAsync(result.val);
      }
    });
  ...
  return (
    <div className={className}>
      {showConnectButton === true && <a href="/login">Connect to Spotify</a>}
      {profileBadgeProps && <ProfileBadge {...profileBadgeProps}></ProfileBadge>}
    </div>
  );
}
